...
class MultiUAVVisualizer:
    def __init__(self, num_uavs=10):
        ...
        for i in range(1, num_uavs + 1):
            topic_name = f'/px4_{i}/mavros/local_position/pose'
            self.paths[i] = Path()
            self.paths[i].header.frame_id = "map"

            rospy.Subscriber(topic_name, PoseStamped, self.pose_callback, i)

            self.path_pubs[i] = rospy.Publisher(f'/uav{i}_path', Path, queue_size=10)
    ...
    def publish_uav_icon(self, uav_id, pose):
        icon_marker = Marker()
        icon_marker.header.frame_id = "map"
...
if __name__ == '__main__':
    try:
        visualizer = MultiUAVVisualizer(num_uavs=10)
        rospy.spin()
